...
const engramCanisterId = computed(
  () => engram.value?.canisterId?.toText?.() || engram.value?.canisterId?.toString() || null,
);
...
const updateAvailable = computed(
  () =>
    engramWasmHash.value &&
    registryWasmHash.value &&
    engramWasmHash.value !== registryWasmHash.value,
);
...
async function executeConfirmedAction() {
  ...
  try {
    if (confirmAction.value === 'claim') {
      ...
      if ('Ok' in result) {
        success.value =
          'Sovereignty claimed. Registry removed as controller. Auto-updates disabled.';
      ...
      try {
        ...
        if ('Ok' in result) {
          success.value =
            'Returned to managed mode. Registry restored as controller and auto-updates re-enabled.';
          await loadSovereignty();
        } else {
        ...
        error.value =
          'Controllers updated, but registry confirmation failed: ' + (e.message || String(e));
      }
    }
  } catch (e: any) {
...
async function loadEntries() {
  ...
  try {
    ...
    entries.value = await engramActor.value.readAuditLog(
      BigInt(page.value * pageSize),
      BigInt(pageSize),
    );
  } finally {
...
<template>
  ...
  <div v-else class="max-w-7xl mx-auto px-4 py-8">
    ...
    <div
      ...
      class="bg-engram-900/20 border border-engram-700/50 rounded-lg p-4 mb-6 flex items-center justify-between gap-4"
    >
      <div>
        <p class="text-engram-300 text-sm font-medium">A new engram version is available</p>
        <p class="text-gray-500 text-xs mt-1">
          Current: {{ engramWasmHash?.slice(0, 12) }}... &rarr; Latest:
          {{ registryWasmHash?.slice(0, 12) }}...
        </p>
      </div>
      <button class="btn-primary text-sm shrink-0" :disabled="upgrading" @click="upgradeMyEngram">
        ...
        <div v-else class="space-y-4">
          <div v-for="entry in changelogEntries" :key="entry.version" class="card">
            <div class="flex items-center gap-3 mb-3">
              <span
                class="px-2 py-0.5 rounded-full text-xs font-semibold bg-purple-500/20 text-purple-400"
              >
                v{{ entry.version }}
              </span>
          ...
          <div class="card mb-6">
            ...
            <div v-if="sovereigntyStatus === 'Managed'">
              <p class="text-sm text-gray-400 mb-4">
                Your engram is managed by the registry. The registry can push WASM upgrades to keep
                your engram up to date. You can claim sovereignty at any time to take full control
              ...
              <div class="pt-4 border-t border-gray-800">
                ...
                <p class="text-xs text-gray-500 mt-2">
                  Removes the registry as a controller of your engram. You will be the sole
                  controller. Auto-updates will be disabled. You can return to managed mode later.
                </p>
              </div>
            </div>
            ...
            <div v-else-if="sovereigntyStatus === 'Sovereign'">
              <p class="text-sm text-gray-400 mb-4">
                Your engram is sovereign. You are the sole controller. The registry cannot push
                upgrades or modify your engram. To receive auto-updates again, return to managed
                mode.
              </p>
        ...
        <div v-else class="space-y-3">
          <template v-for="entry in entries" :key="Number(entry.id)">
            <div
              ...
              :class="[
                'card border-l-4',
                entry.success ? 'border-l-green-600' : 'border-l-red-600',
              ]"
            >
              <div class="flex justify-between items-start mb-2">
                <span class="font-medium text-white text-sm">{{ entry.operation }}</span>
                <span
                  :class="entry.success ? 'text-green-400' : 'text-red-400'"
              ...
              <div class="text-xs text-gray-500 mb-1">
                <span
                  :class="
                    getRoleString(entry.callerRole) === 'Owner'
                      ? 'text-blue-400'
                      : 'text-purple-400'
                  "
                >
              ...
              <p v-if="entry.details" class="text-sm text-gray-400 break-all">
                {{ entry.details }}
              </p>
            </div>
          </template>
        ...
        <div class="relative bg-gray-900 border border-gray-700 rounded-xl p-6 max-w-md mx-4">
          ...
          <p class="text-sm text-gray-400 mb-6">
            <template v-if="confirmAction === 'claim'">
              This will remove the registry as a controller of your engram. You will be the sole
              controller and will no longer receive automatic upgrades. You can reverse this later
              by returning to managed mode.
            </template>
            <template v-else>
              This will add the registry back as a controller of your engram, allowing it to push
              WASM upgrades. You will remain a controller alongside the registry.
            </template>
          </p>
